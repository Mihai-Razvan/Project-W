...
public class Item_016_SaveData : PlaceableData
{
    ...
    public static List<Item_016_SaveData> getItemsData()        
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
            if (colliders[i] is BoxCollider && ItemsList.getItemCode(colliders[i].gameObject.tag) == 16)  //we check to be a boxCollider in case that object got other colliders to, otherwise the object would be saved once for every collider
            {
